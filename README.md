...
After adding this code, calling the ```setup_backgrounds()``` and ```setup_floor()``` before the game loop and ```draw_backgrounds()``` and ```draw_floor()``` functions in the game loop, and running the script we will now see a much prettier background with our layered fauna and colorful grassy floor! 
